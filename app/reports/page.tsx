...
function getReportsData() {
  const signOuts = db
    .prepare(
      `SELECT s.*, v.number_plate 
       FROM vehicle_sign_outs s 
       JOIN vehicle_entries v ON s.vehicle_id = v.id`
    )
    .all();

  const parkingTotal =
    db.prepare("SELECT SUM(amount) as total FROM parking_transactions")
      .get()?.total || 0;

  const tyreTotal =
    db.prepare("SELECT SUM(amount) as total FROM tyre_service_transactions")
      .get()?.total || 0;

  const batteryTotal =
    db.prepare("SELECT SUM(amount) as total FROM battery_transactions")
      .get()?.total || 0;

  return { signOuts, parkingTotal, tyreTotal, batteryTotal };
}

export default function Reports() {
  const { signOuts, parkingTotal, tyreTotal, batteryTotal } =
    getReportsData();
